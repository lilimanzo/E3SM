    ...
    if (iamin_CPLID) then
       if (ocn_present) then
          ...
          do exi = 1,num_inst_xao
             ...
             call seq_flux_ocnalb_mct(infodata, ocn(1), a2x_ox(eai), fractions_ox(efi), fractions_ax(efi), xao_ox(exi)) ! LM added fractions_ax
          enddo
...
!----------------------------------------------------------------------------------
  ...
  subroutine cime_run_atm_setup_send()
    ...
    if (iamin_CPLID .and. atm_prognostic) then
       ...
       !a2x_ax => component_get_c2x_cx(atm) ! LM added- not sure if it makes a difference
  ...
  subroutine cime_run_calc_budgets2(in_cplrun)
    ...
    if (iamin_CPLID) then
       ...
       if (atm_present) then
          ...
          a2x_ax => component_get_c2x_cx(atm(eai)) ! LM added
          call seq_diag_saf(atm(ens1), fractions_ax(ens1), a2x_ax)  ! LM added
       endif
       if (ice_present) then
