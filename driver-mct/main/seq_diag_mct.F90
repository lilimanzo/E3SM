...
101 format('avloc_diag ',a3,1x,a4,1x,i3,es26.19,1x,a)
  ...
  subroutine seq_diag_saf(atm, frac_a, a2x_a) ! LM created subroutine
        ...
        do n=1,lSize
                ...
                ! (Fup-Fdn) / (sb * (((iFup-(1-ei))/sb)**.25 + ((oFup-(1-eo))/sb)**.25 + ((lFup-(1-1))/sb)**.25 )**4 )
                x2a_a % rAttr(index_x2a_Sx_saf, n) = &
                        (-x2a_a%rAttr(index_x2a_Faxx_lwup,n) - a2x_a%rAttr(index_a2x_Faxa_lwdn,n)) &
                        / (shr_const_stebol * ( &
                        (((-frac_a%rAttr(kir,n)*x2a_a%rAttr(index_x2a_Faxx_lwup,n)) &
                        -(1-x2a_a%rAttr(index_x2a_Sl_taf,n)))/shr_const_stebol)**.25 & 
                        + ((((-frac_a%rAttr(kor,n)*x2a_a%rAttr(index_x2a_Faxx_lwup,n)) &
                        -(1-shr_const_ocn_msv)))/shr_const_stebol)**.25 &
                        + ((((-frac_a%rAttr(kl,n)*x2a_a%rAttr(index_x2a_Faxx_lwup,n)) & 
                        - (1-1))) / shr_const_stebol)**.25) **4)
