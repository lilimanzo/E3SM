    ...
    if (present(do_x2a)) then
       ...
       do n=1,lSize
          do k=1,4
             ...
             nf = f_hlwdn_prev; budg_dataL(nf,ic,ip) = budg_dataL(nf,ic,ip) + ca_a*x2a_a%rAttr(index_x2a_Faxx_lwdn_prev,n) ! LM added
...
101 format('avloc_diag ',a3,1x,a4,1x,i3,es26.19,1x,a)
  ...
  subroutine seq_diag_saf(atm, frac_a, a2x_a) ! LM created subroutine
        ...
        do n=1,lSize
                ...
                ! (Fup-Fdn)/(sb*(((ifrac*(Fup-(1-ei)*Fdn))/(ei*sb))**.25+((ofrac(Fup-(0))/(eo*sb)))**.25+((lFup-(0))/sb)**.25)**4-Fdn)
