    ...
    do icall = 0, N_DIAG
       ...
       if (active_calls(icall)) then
          ...
          call addfld('FLUS_SB'//diag(icall), horiz_only,    'A',    'W/m2', 'LM added surface flux from SB law', &
                      sampling_seq='rad_lwsw', flag_xyfill=.true.)
          call addfld('FLUS_EG'//diag(icall), horiz_only,    'A',    'W/m2', 'LM added diagnostic EG surface flux from SB law', &
                      sampling_seq='rad_lwsw', flag_xyfill=.true.)
          call addfld('FLUS_FG'//diag(icall), horiz_only,    'A',    'W/m2', 'LM added diagnostic FG surface flux from SB law', &
                      sampling_seq='rad_lwsw', flag_xyfill=.true.)
          ...
          !            sampling_seq='rad_lwsw', flag_xyfill=.true.)
          !call addfld('LD'//diag(icall), (/'ilev','lwband'/),'I', 'W/m2', 'LM added spectral downwelling flux', &
          !            sampling_seq='rad_lwsw', flag_xyfill=.true.)    
          !call addfld('FLUS_SPC'//diag(icall), (/'ilev','lwband'/),'I', 'W/m2', 'LM added spectral upwelling surface flux', &
          !            sampling_seq='rad_lwsw', flag_xyfill=.true.)
          ...
          if (history_amwg) then
             ...
             !call add_default('LU'//diag(icall),    1, ' ') ! LM added
...
!===============================================================================
  ...
  subroutine radiation_tend(state,ptend, pbuf, &
    ...
    real(r8) fdl(pcols,pverp)     ! LM added column lwdn
    real(r8) fsdl(pcols,pverp)    ! LM added column clearsky lwdn
       ...
       if (dolw) then
          ...
          do icall = N_DIAG, 0, -1
              ...
              if (active_calls(icall)) then
                  ...
                  call rad_rrtmg_lw( & ! LM added flus, flusc, trad, ful, fsul, fdl, fsdl, flus_sb, trad_eg, trad_fg, flus_eg, flus_fg
                       lchnk,        ncol,         num_rrtmg_levs,  r_state,                     &
                       ...
                       trad_eg,      trad_fg,      flus_eg,         flus_fg        ) 
