...
!---------------------------------------------------------------------------
  ...
  type cam_in_t    
     ...
     real(r8), allocatable :: lwup_gb(:)    ! LM added diagnostic greybody flux
...
!
    ...
    do c = begchunk,endchunk 
       ...
       allocate (cam_in(c)%lwup_gb(pcols), stat=ierror)                                ! LM added
       if ( ierror /= 0 ) call endrun('HUB2ATM_ALLOC error: allocation error lwup_gb') !
    ...
    do c = begchunk,endchunk
       ...
       cam_in(c)%lwup     (:) = 0._r8
       cam_in(c)%lwup_gb  (:) = 0._r8 ! LM added
  ...
  subroutine hub2atm_deallocate(cam_in)
    ...
    if(associated(cam_in)) then
       do c=begchunk,endchunk
          ...
          deallocate(cam_in(c)%lwup_gb)   ! LM added
