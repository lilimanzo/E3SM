...
!---------------------------------------------------------------------------
  ...
  type cam_in_t    
     ...
     real(r8), allocatable :: lwdn_prev(:)  ! LM added lw down from prev timestep
...
!
    ...
    do c = begchunk,endchunk 
       ...
       allocate (cam_in(c)%lwdn_prev(pcols), stat=ierror)                                ! LM added
    ...
    do c = begchunk,endchunk
       ...
       cam_in(c)%lwup     (:) = 0._r8
       cam_in(c)%lwdn_prev(:) = 0._r8 ! LM added
       cam_in(c)%lhf      (:) = 0._r8
       cam_in(c)%shf      (:) = 0._r8
  ...
  subroutine hub2atm_deallocate(cam_in)
    ...
    if(associated(cam_in)) then
       do c=begchunk,endchunk
          ...
          deallocate(cam_in(c)%lwdn_prev) ! LM added
